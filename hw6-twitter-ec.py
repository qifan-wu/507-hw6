...
def find_top3_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    -------
    string
        the 3 hashtag that most commonly co-occurs with the hashtag 
        queried in make_request_with_cache()

    '''
    hashtag_dic = {}
    for each_tweet in tweet_data["statuses"]:
        for each_hashtag in each_tweet["entities"]["hashtags"]:
            hashtag_name = each_hashtag["text"]
            if hashtag_name not in hashtag_dic.keys():
                hashtag_dic[hashtag_name] = 1
            else:
                hashtag_dic[hashtag_name] += 1
    # for k, v in hashtag_dic.items():
    #     if v > 5:
    #         print(k, v)
    for k in hashtag_dic.keys():
        if k.lower() == hashtag_to_ignore[1:].lower():
            hashtag_dic[k] = 0

    sorted_tuples = sorted(hashtag_dic.items(), key=operator.itemgetter(1))
    # print(sorted_tuples)
    sorted_dict = OrderedDict()
    for k, v in sorted_tuples:
        sorted_dict[k] = v
    top1 = list(sorted_dict.items())[-1]
    top2 = list(sorted_dict.items())[-2]
    top3 = list(sorted_dict.items())[-3]
    return f"{top1[0]}: {top1[1]} times; {top2[0]}: {top2[1]} times; {top3[0]}: {top3[1]} times;"
    ...
    while True:
        ...
        if hashtag.lower() == 'exit':
            ...
            if hashtag[0] != '#':
                ...
                hashtag = hashtag
            tweet_data = make_request_with_cache(baseurl, hashtag, count)
            most_common_3 = find_top3_common_cooccurring_hashtag(tweet_data, hashtag)
            if most_common_3 is None:
                print("No result found!")
            else:
                print("The most 3 commonly cooccurring hashtag with {} is {}.".format(hashtag, most_common_3))